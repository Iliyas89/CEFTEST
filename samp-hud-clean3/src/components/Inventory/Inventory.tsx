...
const Inventory: React.FC<InventoryProps> = ({ onClose }) => {
    ...
    const sendToServer = (eventName: string, data: any = {}) => {
        if ((window as any).cef) {
            ...
            if (eventName === 'onMoveItem' && data.from !== undefined && data.to !== undefined) {
                payload = `${data.from} ${data.to}`;
            }
            else if (eventName === 'onItemAction' && data.action && data.slotIndex !== undefined) {
                payload = `${data.action} ${data.slotIndex}`;
            }
            ...
            if (payload) {
                (window as any).cef.emit(eventName, payload);
            } else {
                (window as any).cef.emit(eventName);
            }
        } else {
    ...
    useEffect(() => {
        // ИСПРАВЛЕНО: Безопасный приём данных от CEF (с разбором массивов аргументов)
        const handleUpdateInventory = (...args: any[]) => {
            try {
                let rawData = args[0];
                // Если плагин CEF передал данные внутри массива, достаем первый элемент
                ...
                if (parsed && parsed.slots) {
                    setInventorySlots(parsed.slots);
                }
                if (parsed && parsed.equipment) {
                    setEquipment(prev => ({ ...prev, ...parsed.equipment }));
                }
            } catch (e) {
                console.error('Ошибка парсинга данных инвентаря. Данные:', args, e);
            }
        };

        if ((window as any).cef) {
            (window as any).cef.on('updateInventory', handleUpdateInventory);
            (window as any).cef.on('openInventory', (...args: any[]) => {
                if (args.length > 0) handleUpdateInventory(...args);
            });
        }
        ...
        return () => {
            ...
            if ((window as any).cef) {
                (window as any).cef.off('updateInventory');
                (window as any).cef.off('openInventory');
            }
        };
    }, [onClose]);
            ...
            <div className={styles.rightPanel}>
                <div className={styles.rightHeader}>
                    ...
                    <div className={styles.headerControls}>
                        ...
                        <button className={`${styles.controlBtn} ${styles.closeBtn}`} onClick={handleCloseClick}>✕</button>
                    </div>
                </div>
